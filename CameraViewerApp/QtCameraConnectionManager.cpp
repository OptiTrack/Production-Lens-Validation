...
void CameraConnectionManager::CameraInitialized() {
  if (SyncCameraConnections() > 0) {
    QMetaObject::invokeMethod(
        this, [this] { emit camerasChanged(); }, Qt::QueuedConnection);
...
bool CameraConnectionManager::WaitForNewCamera(int timeoutMs) {
  ...
  bool ok =
      notify_condition.wait_for(lock, std::chrono::milliseconds(timeoutMs),
...
CameraConnectionManager::GetCameras() const {
  return current_cameras;
}

std::shared_ptr<const Frame>
CameraConnectionManager::GetLatestFrame(unsigned int serial) const {
  auto cam = CameraManager::X().GetCameraBySerial(serial);
  return cam ? cam->LatestFrame() : nullptr;
}

std::shared_ptr<const Frame>
CameraConnectionManager::GetNextFrame(unsigned int serial) const {
  auto cam = CameraManager::X().GetCameraBySerial(serial);
...
CameraConnectionManager::GetCamera(unsigned int serial) const {
  auto cam = CameraManager::X().GetCameraBySerial(serial);
  return cam;
}
...
bool CameraConnectionManager::SetColorGamma(unsigned int serial, float gamma) {
  ...
  if (!cam)
    return false;
  if (!CameraHelper::IsPrimeColor(*cam))
    return false; // not supported
  cam->SetColorGamma(gamma);
  return true;
}

bool CameraConnectionManager::SetColorCompression(unsigned int serial, int mode,
                                                  float quality,
                                                  float bitrateScaled) {
  auto cam = GetCamera(serial);
  if (!cam)
...
bool CameraConnectionManager::SetVideoType(unsigned int serial,
                                           Core::eVideoMode mode,
                                           QString *outError) {
  auto cam = GetCamera(serial);
  if (!cam) {
    if (outError)
      *outError = "No camera for that serial.";
    return false;
  }
  ...
  if (mode == Core::DuplexMode && !isXCamera) {
    if (outError)
      *outError = "Duplex mode is only supported on X cameras.";
    return false;
  }
  ...
  if (isPrimeColor && mode != Core::VideoMode) {
    if (outError)
      *outError = "Prime Color cameras only support Color Video mode via "
