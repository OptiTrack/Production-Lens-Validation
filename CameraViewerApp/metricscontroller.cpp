      ...
      if (metrics.contains(key)) {
        ...
        const QString unitsSuffix =
            metricWidgets->units.isEmpty() ? QString()
                                           : QStringLiteral(" ") +
                                                 metricWidgets->units;
        dataLabel->setText(QString::number(value * 100, 'f', 2) + unitsSuffix);

        // Colour comes from motive.css; the font size is already set on the
        // label's QFont, so there is no stylesheet to rebuild per update.
        const bool passing = value >= metricWidgets->passingThreshold;
        ui::setStatus(dataLabel,
                      passing ? ui::Status::Pass : ui::Status::Neutral);
