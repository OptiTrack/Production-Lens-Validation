...
namespace {
// Return bytes-per-pixel for the subset of target pixel formats we support
// writing into
...
cModuleVideoDecompressorLibav::cModuleVideoDecompressorLibav()
    : color_conversion_context(nullptr), libav_initialized(false),
      libav_decode_frame(nullptr), libav_frame(nullptr), av_decoder(nullptr),
      decoder_context(nullptr) {}

cModuleVideoDecompressorLibav::~cModuleVideoDecompressorLibav() { Shutdown(); }

bool cModuleVideoDecompressorLibav::PostVideoData(
    Camera *, const unsigned char *Buffer, long bufferSize, const Frame *,
    int frameWidth, int frameHeight, unsigned char *alignedFrameBuffer,
    long alignedFrameBufferSize) {
  return PostVideoData(nullptr, Buffer, bufferSize, nullptr, frameWidth,
                       frameHeight, alignedFrameBuffer, alignedFrameBufferSize,
                       AV_PIX_FMT_RGBA);
}
...
bool cModuleVideoDecompressorLibav::PostVideoData(
    Camera *, const unsigned char *Buffer, long bufferSize, const Frame *,
    int frameWidth, int frameHeight, unsigned char *alignedFrameBuffer,
    long alignedFrameBufferSize, int pixelFormat) {
  ...
  if (!libav_initialized) {
    ...
    if (alignedFrameBuffer && alignedFrameBufferSize > 0)
      memset(alignedFrameBuffer, 0,
             static_cast<size_t>(alignedFrameBufferSize));
    return false;
  }

  if (!Buffer || bufferSize <= 0 || !alignedFrameBuffer || frameWidth <= 0 ||
      frameHeight <= 0) {
    return false;
  }
  ...
  color_conversion_context = sws_getCachedContext(
      color_conversion_context, srcW, srcH, srcFmt, frameWidth, frameHeight,
      dstFmt, SWS_BILINEAR, nullptr, nullptr, nullptr);
  ...
  ret = av_image_fill_arrays(libav_frame->data, libav_frame->linesize,
                             alignedFrameBuffer, dstFmt, frameWidth,
                             frameHeight, 1);
  ...
  const int scaled =
      sws_scale(color_conversion_context, libav_decode_frame->data,
                libav_decode_frame->linesize, 0, srcH, libav_frame->data,
...
unsigned char *
cModuleVideoDecompressorLibav::AlignedMemoryAllocation(long size) {
