...
QtCameraViewer::QtCameraViewer(
    ...
    CameraHelper::FrameRateCalculator &fpsCalc, DisplayResults *newText,
    MetricsExporter &metricsExporter, QWidget *parent)
    : QWidget(parent), camera_manager(mgr), camera_mutex(camMutex),
      ...
      focus_result(newText), metrics_exporter(metricsExporter) {
  buildUi();
  wireSignals();
}
...
void QtCameraViewer::buildUi() {
  ...
  camera_controls =
      new CameraControlPanel(camera_manager, metrics_exporter, this);
  ...
  // Row 3: Another status bar, this time with focus eval result (red, green,
  // yellow)
...
void QtCameraViewer::retranslateUi() {
  ...
  if (tab0_visibility_button) {
    tab0_visibility_button->setText(
        QCoreApplication::translate("QtCameraViewer", "Control Tab"));
  }
  if (tab1_visibility_button) {
    tab1_visibility_button->setText(
        QCoreApplication::translate("QtCameraViewer", "Video Modes Tab"));
  }
  if (tab2_visibility_button) {
    tab2_visibility_button->setText(
        QCoreApplication::translate("QtCameraViewer", "Color Tab"));
  }
  ...
  if (tab4_visibility_button) {
    tab4_visibility_button->setText(
        QCoreApplication::translate("QtCameraViewer", "Exporter Tab"));
  }
  if (empty_label) {
    empty_label->setText(
  ...
  if (language_combo && language_combo->count() >= 2) {
    language_combo->setItemText(
        0, QCoreApplication::translate("QtCameraViewer", "English"));
    language_combo->setItemText(
        1, QCoreApplication::translate("QtCameraViewer", "Simplified Chinese"));
  }
  if (camera_picker) {
