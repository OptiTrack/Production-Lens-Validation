...
void VideoWidget::SetupLockOverlay() {
  ...
  lockImg = QImage(QSize(svgSize.width() * 3, svgSize.height() * 3),
                   QImage::Format_RGBA8888); // RGBA8 for OpenGL
...
void VideoWidget::initializeGL() {
  ...
  // This will be replaced every frame if edge detection is enabled with real
  // edge map data via OpenCV.
...
void VideoWidget::paintGL() {
  ...
  if (has_pending.exchange(false)) {
    ...
    if (w > 0 && h > 0 && src) {
      ...
      if (bytesPerPixel > 0) {
        ...
        const GLint rowLenPixels =
            (bytesPerPixel > 0) ? (srcStride / bytesPerPixel) : 0;
        ...
        const bool formatChanged =
            (internalFormat != current_internal_format) ||
            (format != current_format) || (type != current_type);

        if (sizeChanged || formatChanged) {
          glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, w, h, 0, format, type,
                       src);
          texture_width = w;
          texture_height = h;
  ...
  program_shader->setUniformValue(edge_color_uniform,
                                  QVector3D(0.00f, 0.65f, 1.0f));

  // Only show overlay when edge detection is enabled: alpha = 1.0 (showing)
  // else 0.0 (transparent)
  const float alpha =
      edge_detect_enabled.load(std::memory_order_relaxed) ? 1.0f : 0.0f;
  ...
  // (disabled when ROI zoom is active)
  {
    ...
    if (!roiEnabled && circleEnabled && !detectedCircleMarkers.empty()) {
      ...
      if (circleMarkersTex != 0) {
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0, 0);
        glVertex2f(dstX, glYBottom); // bottom-left
        glTexCoord2f(1, 0);
        glVertex2f(dstX + dstW, glYBottom); // bottom-right
        glTexCoord2f(1, 1);
        glVertex2f(dstX + dstW, glYTop); // top-right
        glTexCoord2f(0, 1);
        glVertex2f(dstX, glYTop); // top-left
        glEnd();
...
void VideoWidget::updateFrameFromBitmap(CameraLibrary::Bitmap *bmp) {
  ...
  const int bytesPerPixel = (bpp == 8)    ? 1
                            : (bpp == 16) ? 2
                            : (bpp == 24) ? 3
                                          : 4;
  ...
  const bool edgeDetectEnabled =
      edge_detect_enabled.load(std::memory_order_relaxed);
  const bool roiEnabled = roiZoomEnabled.load(std::memory_order_relaxed);
  ...
  if (edgeDetectEnabled || roiEnabled) {
    ...
    cv::Mat sourceMat;
    // Wrap source buffer in a cv::Mat. Treating it as read-only; OpenCV never
    // writes to this buffer.
    if (bpp == 8) {
      sourceMat =
          cv::Mat(h, w, CV_8UC1, const_cast<unsigned char *>(src), srcStride);
    } else if (bpp == 16) {
      sourceMat =
          cv::Mat(h, w, CV_16UC1, const_cast<unsigned char *>(src), srcStride);
    } else if (bpp == 24) {
      sourceMat =
          cv::Mat(h, w, CV_8UC3, const_cast<unsigned char *>(src), srcStride);
    } else if (bpp == 32) {
      sourceMat =
          cv::Mat(h, w, CV_8UC4, const_cast<unsigned char *>(src), srcStride);
    }
    ...
    if (roiEnabled) {
      ...
      if (!combined.empty()) {
        ...
        for (int y = 0; y < h; ++y) {
          unsigned char *d =
              reinterpret_cast<unsigned char *>(byte_array_staging.data()) +
              y * dstStride;
...
void VideoWidget::mousePressEvent(QMouseEvent *event) {
  ...
  // fall back to a simple quadrant split for the four corner slots.
  int quadrant = -1;

  if (roiZoomEnabled.load(std::memory_order_relaxed) && shapeParams.isValid &&
      shapeParams.combinedW > 0 && shapeParams.combinedH > 0) {
    // Remap the frame-pixel click into combined-image coordinates so we can
    // test it against the diamond polygon (which lives in combined-image
    // space).
    ...
    if (cv::pointPolygonTest(shapeParams.diamondPts, cv::Point2f(cx, cy),
                             false) >= 0.0f) {
  ...
  /*
      ...
      When ROI zoom is active each panel shows a zoomed crop of the original
     image, resized to fill the quadrant. mapClickToImageCoords reverses that
     transform to produce the position of the clicked point in the original
     (un-zoomed) camera image.

      On right-click we skip this and store point (-1,-1) to clear the pinned
     marker instead.
  */
  ...
  qDebug("Clicked pixel: (%d, %d) -> image: (%d, %d) in quadrant %d", px, py,
         newPoint.x, newPoint.y, quadrant);
...
//
// When ROI zoom is active, each quadrant shows a small square crop of the
// original image that has been zoomed in and then stretched to fill the panel
// (panelW x panelH pixels). A user click at panel-local position (lx, ly)
// therefore corresponds to some point inside that original crop window — this
// function works out which one.
//
//   prevCentroid — the zoom center used when the panel was rendered (in
//   original image coords) lx, ly       — click position relative to the
//   top-left corner of the panel (combined-image coords) panelW/H     — pixel
//   dimensions of the panel in the combined image
//
// Returns: Corresponding point in original image coordinates
cv::Point2f VideoWidget::inverseZoomCrop(cv::Point2f prevCentroid, float lx,
                                         float ly, int panelW,
                                         int panelH) const {

  // Reproduce the same crop window that zoomCrop used when rendering this
  // panel. side = the square crop size in the original image (smaller = more
  // zoomed in).
  int side =
      static_cast<int>(std::min(frame_width, frame_height) / ROIZoomScale);

  // Clamp the crop origin so the window doesn't go outside the image (same
  // logic as zoomCrop).
  int cropX =
      std::max(0, std::min(int(prevCentroid.x) - side / 2, frame_width - side));
  int cropY = std::max(
      0, std::min(int(prevCentroid.y) - side / 2, frame_height - side));

  // The panel stretches the crop window over (panelW x panelH) pixels, so
  // divide by that scale to convert panel-local pixels back to original-image
  // pixels.
  return {cropX + lx * float(side) / float(panelW),
          cropY + ly * float(side) / float(panelH)};
}

// Converts a click at frame-pixel position (px, py) back to original image
// coordinates.
//
// When ROI zoom is active, what the user sees on screen is NOT the raw camera
// image. The view is a combined image made of five zoomed panels that has then
// been rescaled to fit the frame dimensions. A click therefore lands somewhere
// inside one of those panels, and we need to undo two layers of scaling to find
// where the user actually clicked in the original camera image:
//
//   Layer 1: frame to combined image  (the combined image was resized to frame
//   dimensions) Layer 2: combined image to original image  (each panel is a
//   zoomed crop, via inverseZoomCrop)
//
// If ROI zoom is not active, or the clicked quadrant has never acquired a
// marker (no zoom center to invert from), the frame-pixel coordinates are
// returned unchanged.
cv::Point2f VideoWidget::mapClickToImageCoords(int px, int py,
                                               int quadrant) const {
  if (!roiZoomEnabled.load(std::memory_order_relaxed) || !shapeParams.isValid ||
      shapeParams.combinedW <= 0 || shapeParams.combinedH <= 0)
  ...
  // Layer 2: subtract the panel's top-left origin within the combined image,
  // then call inverseZoomCrop to undo the zoom+resize for that specific panel.
  if (quadrant == 4 && quadrantSlots[4].hasTrack) {
    // Center diamond panel: its top-left is at (diamondX, diamondY) in
  ...
  if (quadrant >= 0 && quadrant <= 3 && quadrantSlots[quadrant].hasTrack) {
    // Corner quadrant panels are tiled: slot (col, row) starts at (col*quadW,
    // row*quadH).
    int col = quadrant % 2;
    int row = quadrant / 2;
...
// CircleMarkerDetector, rather than using the coarser contour centroid from
// extractROIs.
//
// The circle detector runs a separate, more precise detection pass and stores
// its results in detectedCircleMarkers.  This function searches that list for
// the circle closest to the current quadrant centroid that also falls within
// the quadrant's visible crop window, then returns its center.  If no such
// circle exists the original centroid is returned unchanged, so the quadrant
// continues to use the contour-based position.
//
// The crop window constraint prevents snapping to a circle from a different
// marker that happens to be closer in absolute image distance but is not
// actually visible in this panel.
cv::Point2f VideoWidget::snapCentroidToCircle(cv::Point2f centroid) {
  // Reproduce the crop window size from zoomCrop so we know the maximum
  // distance a circle can be from the centroid and still be visible inside the
  // zoomed panel.
  const int side =
      static_cast<int>(std::min(frame_width, frame_height) / ROIZoomScale);
  const float maxDistSq =
      float(side / 2) * float(side / 2); // squared to avoid sqrt

  std::lock_guard<std::mutex> lock(circleMarkersMutex);

  float bestDistSq = maxDistSq; // reject any circle farther away than the crop
                                // window half-side
...
// OVERVIEW
// ---------
// The combined image is a single cv::Mat that tiles five zoomed views of the
// camera frame:
//
//   ┌────────────┬────────────┐
//   │  TL (0)    │  TR (1)    │
...
//   └────────────┴────────────┘
//
// Each corner panel (quadrants 0-3) is a zoomed crop centered on the outermost
// marker dot in that corner of the original image. The center diamond panel
// (quadrant 4) shows the center marker. Zooming in lets the user inspect
// circularity and focus quality up close.
//
// QUADRANT NUMBERING
// -------------------
...
//   centroid    — the zoom center in original image coordinates (updated every
//   frame) circularity — how circular the tracked marker contour is (0=line,
//   1=perfect circle) hasTrack    — whether the quadrant has ever acquired a
//   marker this session
//
// RETURNS
// ---------
// The combined image WITHOUT any overlay shapes drawn on it.
// Shapes (grid lines, diamond border, circularity labels) are drawn afterward
// in drawShapesOverlay() so that edge detection sees clean pixel data.
//
// Returns an empty Mat if no edges were found.
cv::Mat VideoWidget::applyRoiZoomToFrame(unsigned char *src, cv::Mat &gray,
  ...
  cv::Mat combined(combinedH, combinedW, gray.type(),
                   cv::Scalar(0)); // black canvas
  ...
  // Diamond polygon vertices in diamond-local coordinates (origin = diamondX,
  // diamondY). Used both for masking the rendered crop and for hit-testing
  // mouse clicks.
  ...
  // Run Canny on the grayscale frame to find edges, then extractROIs groups
  // those edges into individual marker blobs with bounding rects, centroids,
  // and circularity.
  cv::Mat edges;
  ...
  if (!edges.empty()) {
    ...
    if (!rois.empty()) {
      ...
      // Quadrant 4 (center diamond): the ROI whose centroid is closest to the
      // image center.
      int centerIdx = -1;
      {
      ...
      // Quadrants 0-3 (corner panels): bin every other ROI into the quadrant
      // that contains its centroid. The center marker (centerIdx) is excluded
      // from this pool.
      ...
      /*
          --- Select the best candidate for the four square quadrants and update
         centroid ---

          Priority order for choosing which candidate to zoom into:
          1. Manual pin  — user clicked a specific marker; pick the ROI closest
         to that point.
          2. Active track — quadrant has a marker, so pick the candidate closest
         to the last-known centroid (frame-to-frame continuity). If nothing is
         close enough the track is considered lost.
          3. First acquisition / track lost — pick the candidate closest to this
         quadrant's corner of the image (i.e. the outermost marker in that
         corner).

          Once a candidate is chosen, the centroid is updated via exponential
         moving average (EMA) to smooth jitter. A manual pin bypasses EMA and
         locks the centroid directly.
      */
      int id = 0;
      for (int slot = 0; slot < 4; ++slot) {
        ...
        // Priority 1: manual pin — find the candidate nearest the clicked
        // point.
        ...
        // Priority 2: active track - find the candidate nearest the previous
        // centroid.
          ...
          if (minDist >= centroid_matching_threshold)
            chosen = -1; // too far - track lost
        }

        // Priority 3: no track — pick the candidate closest to this slot's
        // image corner
        if (chosen < 0) {
          int col = slot % 2;
          int row = slot / 2;
        ...
        // tighter zoom. Only done when the user has not manually pinned this
        // slot.
        if (quadrantClickPositions[slot].x == -1 &&
            circleDetectionEnabled.load(std::memory_order_relaxed)) {
      ...
      for (int slot = 0; slot < 4; ++slot) {
        ...
        // Crop a square region around the tracked centroid and stretch it to
        // panel size.
        cv::Mat cropped =
            zoomCrop(gray, quadrantSlots[slot].centroid, ROIZoomScale);
        cv::Mat resized;
        cv::resize(cropped, resized, cv::Size(quadW, quadH));
        resized.copyTo(combined(cv::Rect(x, y, quadW, quadH)));

        // Record the panel center and circularity so drawShapesOverlay can
        // label it.
        shapeParams.roiLabels.push_back(
            {cv::Point2f(float(x + quadW / 2), float(y + quadH / 2)),
             quadrantSlots[slot].circularity, id});
        id++;
      }
      ...
      if (centerIdx >= 0) {
        ...
        // Crop and resize the center marker view, then copy it into the
        // combined image through a diamond-shaped mask so only the diamond area
        // is filled.
        ...
        cv::fillPoly(diamondMask,
                     std::vector<std::vector<cv::Point>>{diamondPts},
                     cv::Scalar(255));
        centerResized.copyTo(
            combined(cv::Rect(diamondX, diamondY, diamondW, diamondH)),
            diamondMask);

        shapeParams.roiLabels.push_back(
            {cv::Point2f(float(diamondX + diamondW / 2),
                         float(diamondY + diamondH / 2)),
             s.circularity, id});
      } else {
    ...
    // Cache all layout geometry needed by drawShapesOverlay() and
    // mapClickToImageCoords(). These values are derived from the combined image
    // dimensions computed above and must stay in sync with what was actually
    // rendered this frame.
    ...
    // Translate diamond vertices from diamond-local coords to combined-image
    // coords so pointPolygonTest in mousePressEvent can use them directly.
    shapeParams.diamondPts.clear();
    for (auto &p : diamondPts) {
      shapeParams.diamondPts.push_back(
          cv::Point(p.x + diamondX, p.y + diamondY));
    }
...
// Extracts a square crop from `src` centered on point `center` and zoomed in by
// `zoom` amount.
//
// A zoom of 1.0 would crop the entire shorter dimension of the image (no zoom).
// A zoom of 2.0 crops half that area, making objects appear twice as large when
// the result is stretched back to the same display size.  Higher zoom = tighter
// crop.
//
// The crop is always square (side = min(src width, src height) / zoom) so that
// stretching it to a rectangular panel doesn't add extra distortion beyond the
// aspect ratio.
cv::Mat VideoWidget::zoomCrop(const cv::Mat &src, const cv::Point2f &center,
                              float zoom) {
...
/// <summary>
/// Finds all markers in the frame and returns them as a list of RoiInfo
/// structs.
/// </summary>
...
/// <returns></returns>
std::vector<RoiInfo> VideoWidget::extractROIs(const cv::Mat &gray,
                                              const cv::Mat &edges, int margin,
                                              size_t maxROIs) {
  ...
  for (auto &c : contours) {
    ...
    // Use ellipse-fitting method for consistency with CircleMarkerDetector (0-1
    // scale)
    ...
    if (circleDetectionEnabled.load(std::memory_order_relaxed)) {
      if (c.size() >= 5) {
        try {
          ...
          if (majorAxis > 0) {
            circularity = std::clamp(static_cast<double>(minorAxis / majorAxis),
                                     0.0, 1.0);
          }
        } catch (...) {
          circularity = 0.0;
  ...
  // Sort largest-area first so that the most prominent markers appear at the
  // front of the list, then truncate to the caller's requested cap.
...
/// </summary>
void VideoWidget::drawShapesOverlay(float dstX, float dstY, float dstW,
                                    float dstH) {
  ...
  auto toScreenX = [&](float x) {
    return dstX + x * resizeScaleX * screenScaleX;
  };
  auto toScreenY = [&](float y) {
    return dstY + y * resizeScaleY * screenScaleY;
  };
  ...
  // Left (vertical)
  glVertex2f(toNdcX(toScreenX(diamondLeft), W),
             toNdcY(toScreenY(centerY - smallTickLenPx / 2), H));
  glVertex2f(toNdcX(toScreenX(diamondLeft), W),
             toNdcY(toScreenY(centerY + smallTickLenPx / 2), H));
  // Right (vertical)
  glVertex2f(toNdcX(toScreenX(diamondRight), W),
             toNdcY(toScreenY(centerY - smallTickLenPx / 2), H));
  glVertex2f(toNdcX(toScreenX(diamondRight), W),
             toNdcY(toScreenY(centerY + smallTickLenPx / 2), H));
  // Top (horizontal)
  glVertex2f(toNdcX(toScreenX(centerX - smallTickLenPx / 2), W),
             toNdcY(toScreenY(diamondTop), H));
  glVertex2f(toNdcX(toScreenX(centerX + smallTickLenPx / 2), W),
             toNdcY(toScreenY(diamondTop), H));
  // Bottom (horizontal)
  glVertex2f(toNdcX(toScreenX(centerX - smallTickLenPx / 2), W),
             toNdcY(toScreenY(diamondBottom), H));
  glVertex2f(toNdcX(toScreenX(centerX + smallTickLenPx / 2), W),
             toNdcY(toScreenY(diamondBottom), H));

  // Large tick marks (further out)
  // Left (vertical)
  glVertex2f(toNdcX(toScreenX(diamondLeft - tickHSeparationPx), W),
             toNdcY(toScreenY(centerY - largeTickLenPx / 2), H));
  glVertex2f(toNdcX(toScreenX(diamondLeft - tickHSeparationPx), W),
             toNdcY(toScreenY(centerY + largeTickLenPx / 2), H));
  // Right (vertical)
  glVertex2f(toNdcX(toScreenX(diamondRight + tickHSeparationPx), W),
             toNdcY(toScreenY(centerY - largeTickLenPx / 2), H));
  glVertex2f(toNdcX(toScreenX(diamondRight + tickHSeparationPx), W),
             toNdcY(toScreenY(centerY + largeTickLenPx / 2), H));
  // Top (horizontal)
  glVertex2f(toNdcX(toScreenX(centerX - largeTickLenPx / 2), W),
             toNdcY(toScreenY(diamondTop - tickVSeparationPx), H));
  glVertex2f(toNdcX(toScreenX(centerX + largeTickLenPx / 2), W),
             toNdcY(toScreenY(diamondTop - tickVSeparationPx), H));
  // Bottom (horizontal)
  glVertex2f(toNdcX(toScreenX(centerX - largeTickLenPx / 2), W),
             toNdcY(toScreenY(diamondBottom + tickVSeparationPx), H));
  glVertex2f(toNdcX(toScreenX(centerX + largeTickLenPx / 2), W),
             toNdcY(toScreenY(diamondBottom + tickVSeparationPx), H));
  ...
  if (!shapeParams.roiLabels.empty()) {
...

    {
      ...
      for (int idx = 0; idx < static_cast<int>(shapeParams.roiLabels.size());
        ...
        QString circularityText = QString("c:%1%\nID:%2")
                                      .arg(lbl.circularity * 100.0f, 0, 'f', 1)
                                      .arg(lbl.id);

        QRect textRect = fm.boundingRect(QRect(0, 0, 200, 100), // max bounds
                                         Qt::TextWordWrap, circularityText);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width(), height(), 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, flipped.bits());

    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glEnable(GL_TEXTURE_2D);
    glColor4f(1.f, 1.f, 1.f,
              1.f); // reset color so texture isn't modulated by gray line color
...
void VideoWidget::drawCircleMarkers(float dstX, float dstY, float dstW,
                                    float dstH) {
  // Disable circle marker rendering when ROI zoom is enabled or circle
...
void VideoWidget::updateCircleMarkersTexture() {
  ...
  try {
    if (frame_width > 4096 || frame_height > 4096) {
      qWarning("[videowidget] Frame dimensions too large for marker texture: "
               "%d x %d",
    ...
    qWarning("[videowidget] Exception in updateCircleMarkersTexture: %s",
             e.what());
    if (circleMarkersTex != 0) {
      glDeleteTextures(1, &circleMarkersTex);
      circleMarkersTex = 0;
