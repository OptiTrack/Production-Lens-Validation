...
void GraphWidget::drawMarker(qreal x, qreal y, float ringXRad, float ringYRad,
...
void GraphWidget::paintGL() {
  ...
  glLoadIdentity();
  glOrtho(xScrollOffset - xPadding, xScrollOffset + xWindowSize + xPadding,
          yMin, yMax, -1.0, 1.0);
  ...
  if (lastX >= xScrollOffset && lastX <= xScrollOffset + xWindowSize) {
    drawMarker(lastX, lastY, ringRadiusX, ringRadiusY, diskRadiusX,
               diskRadiusY);
  }
}
